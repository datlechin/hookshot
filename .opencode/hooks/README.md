...
```json
{
  "hooks": {
    "pre-tool-use": {
      "Bash": {
        "enabled": true,
        "script": ".opencode/hooks/bash-worktree-fix.sh",
...
.opencode/hooks/bash-worktree-fix.sh "ls -la"

# Test in worktree (should inject cd)
cd /path/to/worktree
.opencode/hooks/bash-worktree-fix.sh "npm install"
...
1. **Verify the hook is executable:**
   ```bash
   chmod +x .opencode/hooks/bash-worktree-fix.sh
   ```
...
3. **Test the hook manually with a sample command:**
   ```bash
   cd /path/to/worktree
   .opencode/hooks/bash-worktree-fix.sh "npm test"
   ```

4. **Check that your settings.json is valid JSON:**
   ```bash
   cat .opencode/settings.json | python -m json.tool
   ```
