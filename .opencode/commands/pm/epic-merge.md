...
feature_list=""
if [ -d ".opencode/epics/$ARGUMENTS" ]; then
  cd .opencode/epics/$ARGUMENTS
...
epic_github_line=$(grep 'github:' .opencode/epics/$ARGUMENTS/epic.md 2>/dev/null || true)
...
# Extract epic issue number
epic_github_line=$(grep 'github:' .opencode/epics/archived/$ARGUMENTS/epic.md 2>/dev/null || true)
if [ -n "$epic_github_line" ]; then
  epic_issue=$(echo "$epic_github_line" | grep -oE '[0-9]+$' || true)
else
  epic_issue=""
