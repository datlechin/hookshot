 ...
 */
export const DetailPanel = forwardRef<DetailPanelHandle, DetailPanelProps>(
  ({ isOpen = false, onClose, selectedRequest = null, loading = false }, ref) => {
    const [activeTab, setActiveTab] = useState<'overview' | 'headers' | 'body' | 'metadata'>(
      'overview'
    )
    ...
    return (
      <aside
        className={`fixed inset-0 lg:relative lg:inset-auto w-full lg:w-[480px] bg-(--surface) border-l border-(--border) flex flex-col z-40 ${animations.slideIn}`}
      >
        {loading ? (
          <>
            {/* Header with close button */}
            <div className="shrink-0 flex items-center justify-between p-2 border-b border-(--border)">
